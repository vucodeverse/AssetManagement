...
-- 1.1 Departments (manager_user_id FK added via ALTER due to circular dependency)
...
CREATE TABLE allocation_request_detail (
    request_detail_id    INT IDENTITY(1,1) PRIMARY KEY,
...
ON asset(department_id, asset_id);

CREATE INDEX idx_return_detail_asset
ON return_request_detail(asset_id, request_id);

CREATE INDEX idx_return_request_status_id
ON return_request(status, request_id);
...
CREATE TABLE asset_handover_detail (
    handover_detail_id      INT IDENTITY(1,1) PRIMARY KEY,
...
CREATE TABLE transfer_order_detail (
    transfer_detail_id           INT IDENTITY(1,1) PRIMARY KEY,
...
-- 3.4 Bảng trung gian: Ánh xạ Xuất kho cho Lệnh Cấp phát (Pick & Pack)
