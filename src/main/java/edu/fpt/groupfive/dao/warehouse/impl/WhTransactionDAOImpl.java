...
public class WhTransactionDAOImpl implements WhTransactionDAO {
    ...
    @Override
    public Map<Integer, List<Integer>> executeInboundTransaction(Integer poId, Integer executedBy,
            List<AssetPlacementPlan> placements, Integer receiptId) {
        ...
        String insertAssetSql = "INSERT INTO asset (asset_name, asset_type_id, purchase_order_detail_id, " +
                "current_status, original_cost, acquisition_date, warranty_start_date, warranty_end_date) " +
                "SELECT ?, ?, ?, 'AVAILABLE', ?, SYSDATETIME(), o.created_at, " +
                "DATEADD(month, ISNULL(qd.warranty_months, 0), o.created_at) " +
                "FROM purchase_orders o " +
                "JOIN purchase_order_details pod ON pod.purchase_order_id = o.purchase_order_id " +
                "JOIN quotation_detail qd ON pod.quotation_detail_id = qd.quotation_detail_id " +
                "WHERE o.purchase_order_id = ? AND pod.purchase_order_detail_id = ?";
        ...
        try {
            ...
            try (
                    ...
                    PreparedStatement psAsset = connection.prepareStatement(insertAssetSql,
                ...
                for (AssetPlacementPlan plan : placements) {
                    ...
                    psAsset.setInt(5, poId);
                    psAsset.setObject(6, plan.poDetailId());
    ...
    @Override
    public void executeReturnInboundTransaction(Integer handoverId, Integer assetId, Integer zoneId, Integer executedBy,
            String note, Integer receiptId) {
        ...
        try {
            ...
            try (
                ...
                try (PreparedStatement psDelPlace = connection
                        .prepareStatement("DELETE FROM wh_asset_placement WHERE asset_id = ?")) {
    ...
    @Override
    public void executeOutboundTransaction(Integer handoverId, Integer assetId, Integer zoneId, Integer executedBy,
            String note) {
        executeOutboundTransactionInternal(handoverId, assetId, zoneId, executedBy, note, null);
    }

    @Override
    public void executeOutboundTransactionWithReceipt(Integer receiptId, Integer assetId, Integer zoneId,
            Integer executedBy, String note) {
        // Find handoverId from receipt if possible, or just pass null to map if not
        // needed
        Integer handoverId = null;
        try (Connection conn = databaseConfig.getConnection();
                PreparedStatement ps = conn
                        .prepareStatement("SELECT asset_handover_id FROM wh_receipts WHERE receipt_id = ?")) {
    ...
    private void executeOutboundTransactionInternal(Integer handoverId, Integer assetId, Integer zoneId,
            Integer executedBy, String note, Integer receiptId) {
    ...
    @Override
    public List<edu.fpt.groupfive.dto.response.warehouse.InboundSummaryResponseDTO.AssetGroupDTO> findAssetGroupsByReceiptId(
            Integer receiptId) {
        String sql = """
        ...
        try (Connection conn = databaseConfig.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    edu.fpt.groupfive.dto.response.warehouse.InboundSummaryResponseDTO.AssetDetailDTO asset = edu.fpt.groupfive.dto.response.warehouse.InboundSummaryResponseDTO.AssetDetailDTO
                            .builder()
        ...
        for (Map.Entry<String, List<edu.fpt.groupfive.dto.response.warehouse.InboundSummaryResponseDTO.AssetDetailDTO>> entry : grouped
                .entrySet()) {
